...
const UserProfileModal = ({ user, onClose, socket }) => {
    const { api, user: currentUser } = useContext(AuthContext); // Potřebujeme API

    if (!user) return null;

    const handleUnfriend = async () => {
        if (!confirm(`Opravdu chcete odebrat ${user.username} z přátel? Chat zůstane zachován.`)) return;
        try {
            ...
            alert("Uživatel odebrán.");
            onClose();
        } catch (e) {
            alert("Chyba při odebírání.");
        }
    };
